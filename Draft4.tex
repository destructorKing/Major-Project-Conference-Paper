...
\centering
\footnotesize
\begin{tabular}{|p{0.6cm}|p{1.8cm}|p{2.2cm}|p{1.8cm}|p{1.0cm}|p{2.5cm}|p{2.8cm}|}
\hline
\textbf{Ref.} & \textbf{Author(s)} & \textbf{Method / Model} & \textbf{Dataset} & \textbf{Best Metric} & \textbf{Key Finding} & \textbf{Limitation / Gap} \\
\hline
\cite{b3} & Attal et al. (2015) & k-NN, SVM, RF, HMM; 3 IMU sensors & UCI HAR; 6 subjects & 99\% Acc. & k-NN best for activity recognition & No fall prediction; no real-time deployment \\
\hline
\cite{b25} & Saraubon et al. (2018) & Accel. + CNN acoustic; Raspberry Pi & UCI + recorded sounds & 93\% Acc. & Accel. outperforms acoustic detection & No predictive analytics; no personalization \\
\hline
\cite{b27} & Ambrosini et al. (2019) & Acoustic features; LR, SVM, RF, kNN & 90 Italian elderly subjects & 80\% Acc. & Speech rate / pauses indicate MCI & No real-time system; no multimodal sensing \\
\hline
\cite{b11} & Al-khafajiy et al. (2019) & IoT wearable; SW-SHMS; cloud & Real sensor prototype & $<$1 ms latency & Low-latency remote monitoring & No fall/gait analysis; no AI prediction \\
\hline
\cite{b7} & Bandara et al. (2020) & IoT + MQTT + cloud monitoring & Custom household sensors & -- & Reliable real-time sensor data pipeline & No predictive intelligence; no AI layer \\
\hline
\cite{b14} & Mekruksavanich et al. (2022) & Aggregated residual deep network; IMU & Simulated fall data & -- & Improved fall classification accuracy & No embedded deployment; no fall prediction \\
\hline
\cite{b10} & Mohammed \& Hasan (2022) & IoT sensors; Raspberry Pi; cloud ML & Real physiological sensors & 97\% HR Acc. & Reliable physiological monitoring & No fall or gait analysis; no AI prediction \\
\hline
\cite{b23} & Augastiny et al. (2023) & NodeMCU; multi-sensor IoT; cloud & Real-time IoT sensor data & -- & Real-time alerts and remote access & Threshold-based only; no AI capability \\
\hline
\cite{b16} & Patel et al. (2023) & Review: accel., gyro., ML classifiers & Multiple wearable datasets & -- & User compliance is key challenge & Device dependency; no temporal modeling \\
\hline
\cite{b26} & Huang et al. (2024) & MFCC + POS features; SVM, RF, kNN & 92 Chinese elderly subjects & 80.77\% Acc. & Linguistic features outperform acoustic & No wearable integration; no real-time system \\
\hline
\cite{b17} & Lim et al. (2024) & Computer vision gait features; LightGBM & MMU-FRiP + Mendeley & 96\% Acc. & Temporal gait features predict fall risk & No real-time wearable implementation \\
\hline
\cite{b4} & Kumar et al. (2024) & Accel. + gyro. fusion; threshold + ML & Controlled sensor data & -- & Sensor fusion improves detection & High false positives during ADL \\
\hline
\cite{b33} & Li et al. (2024) & 1D CNN; automatic feature extraction & Wearable accel./gyro. data & High Acc. & CNN outperforms traditional ML & Too complex for low-power edge devices \\
\hline
\cite{b34} & Patel et al. (2024) & RF, SVM, Decision Trees; stat. features & MobiAct, SisFall & -- & ML models outperform threshold methods & Requires extensive feature engineering \\
\hline
\cite{b12} & Rahmani et al. (2024) & IMU + ECG + BVP + EDA multimodal & EmoWear; 49 subjects & -- & Single IMU for gait + emotion sensing & Lab setting; no real-time deployment \\
\hline
\cite{b36} & Lee et al. (2024) & OpenPose + deep learning; vision & Indoor video datasets & High Acc. & High accuracy in controlled environments & Privacy concerns; poor low-light performance \\
\hline
\cite{b37} & Verma et al. (2023) & Hybrid threshold + ML; wearable sensors & Wearable labeled datasets & -- & Hybrid approach reduces false alarms & Simulated data; lacks real-world validation \\
\hline
\cite{b13} & Gaud et al. (2025) & 1D CNN + BiLSTM; dual IMU; SisFall & SisFall dataset & 99.68\% Acc. & Highest detection accuracy reported & Detection only; no fall-risk prediction \\
\hline
\cite{b35} & Sharma et al. (2025) & ESP8266 + Firebase; IoT alert system & IoT cloud sensor data & Low latency & Real-time alerts via mobile app & Network dependent; no on-device AI \\
\hline
\cite{b18} & Tsiakiri et al. (2025) & Bibliometric review; 126 studies (Scopus) & 126 peer-reviewed studies & -- & Gait as digital biomarker for dementia & No standardized real-time clinical system \\
\hline
\cite{b5} & Zhang et al. (2025) & CNN + LSTM; wearable + vision data & Public fall datasets & High Acc. & Improved temporal fall classification & Needs real-world dataset; no edge deployment \\
\hline
\cite{b15} & MDPI Sensors (2025) & Systematic review; 130+ systems compared & Multiple public datasets & $>$95\% (DL) & Hybrid systems outperform standalone & Integration complexity unaddressed \\
\hline
\cite{b21} & Deshmukh (2026) & IoT wearables + ML; cloud risk prediction & IoT physiological sensor data & -- & Early health-risk detection enabled & No gait/behavioral/cognitive monitoring \\
\hline
\cite{b2} & Ishaq et al. (2026) & PRISMA review; 130 studies (2008--2025) & 130 research papers & $>$95\% (DL) & Shift from reactive to predictive detection & Simulated environments; no real deployment \\
\hline
\cite{b22} & Khalil et al. (2026) & Agentic AI; LLMs + sensor fusion & Wearable + EHR + smart home & -- & Proactive personalized elderly care & Limited real-world validation; ethical issues \\
\hline
\multicolumn{7}{|c|}{\textbf{Proposed}} \\
\hline
-- & Guardian 360 & CNN-GRU; ESP32; IMU + PPG + GPS & SisFall, MobiAct, UP-Fall & 96.4\% Acc. & Predictive + real-time edge AI & Real-world validation scope \\
\hline
\end{tabular}
\end{table*}
...
\node[block, fill=blue!15, below left=1.2cm and 0.3cm of esp] (display) {OLED\\Display};
    
\node[block, fill=blue!15, below=1.2cm of esp] (buzzer) {Buzzer /\\Alert};
    
\node[block, fill=blue!15, below right=1.2cm and 0.3cm of esp] (cloud) {Firebase\\Cloud};
...
\bibitem{b27}
E. Ambrosini \textit{et al.}, ``Automatic Speech Analysis to Early Detect Functional Cognitive Decline in Elderly Population,'' in \textit{Proc. 41st Annu. Int. Conf. IEEE EMBC}, Berlin, Germany, Jul. 2019, pp.~4647--4650. [Online]. Available: https://re.public.polimi.it/bitstream/11311/1120545/1/Ambrosini\_EMBC2019\_revised.pdf
